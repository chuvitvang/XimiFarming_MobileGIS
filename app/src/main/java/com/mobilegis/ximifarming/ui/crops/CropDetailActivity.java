package com.mobilegis.ximifarming.ui.crops;
...
import com.mobilegis.ximifarming.databinding.ActivityCropDetailBinding;
import com.mobilegis.ximifarming.databinding.ItemCropLogBinding;
...
public class CropDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cropId = getIntent().getLongExtra("CROP_ID", -1L);
    ...
    private void showAddLogDialog() {
        ...
        builder.setPositiveButton("Lưu", (dialog, which) -> {
            ...
            // Save log and update crop overall status in background thread
            final String finalNotes = notes;
            final String finalStatus = status;
            new Thread(() -> {
                CropLog log = new CropLog(cropId, System.currentTimeMillis(), finalStatus, finalNotes, "");
                db.cropLogDao().insert(log);

                Crop crop = db.cropDao().getCropById(cropId);
                if (crop != null) {
                    crop.setStatus(finalStatus);
                    db.cropDao().update(crop);
                    if (crop.isSynced()) {
                        com.mobilegis.ximifarming.supabase.SupabaseClient.getInstance().updateCropOnSupabase(crop, null);
                    }
                }
                runOnUiThread(() -> {
